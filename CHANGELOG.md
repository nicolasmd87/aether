...
- **`os_execv(prog, argv_list)` — argv-based process replacement**: new `std.os` extern that replaces the current process image with `prog`, passing each element of `argv_list` (a `list<ptr>` of C strings) as an explicit argv entry. No shell in the middle, so paths containing spaces, quotes, or `$`-signs are safe — a persistent quoting-bug surface in any tool that currently builds shell command strings by concatenation. POSIX implementation forwards to `execvp(3)` after `fflush(stdout); fflush(stderr)` so pre-exec diagnostic output is not lost when stdio is fully-buffered (redirected / pipe). `argv_list == null` or an empty list is tolerated — argv[0] is synthesised from `prog` in that case. `prog` is looked up on `PATH` when it does not contain a slash (same contract as POSIX `execvp`). Null entries in the argv list bail early rather than passing `NULL` into `execvp`'s variadic-argv contract, which has undefined behaviour. On success the call never returns; on failure it returns `-1` with the current process still running. Windows returns `-1` unconditionally — callers that need a Windows process replacement should pair `os_run` (PR #148) with `exit(rc)`. List marshalling declares `list_size` / `list_get` as forward externs inside `std/os/aether_os.c` to avoid a dependency cycle on `std/collections/aether_collections.h`. Test: `tests/syntax/test_os_execv.ae` covers a nonexistent program (returns `-1`), a null `argv_list` (synthesised argv[0]), and the happy path of a successful `/bin/echo` replacement whose stdout becomes the test's tail output. Runnable example: `examples/stdlib/process-exec.ae` shows self-path discovery + argv-forwarding to `/bin/echo`. Addresses P3 in aetherBuild's `docs/for-nicolas.md` wishlist. PR #148 covers P1 (`os_run` / `os_run_capture`) and PR #153 covers most of P4 (`fs_mkdir_p`, `fs_symlink`, `fs_readlink`, `fs_is_symlink`, `fs_unlink`, `os_which`); together those three PRs let `aeb` stop shelling out for process launch, self-discovery, and filesystem setup.

- **Security hardening test suite**: 20 C unit tests (`tests/compiler/test_security.c`) covering symbol table NULL safety, hide/seal boundary enforcement, qualified name enforcement, and scope chain stress. 2 Aether acceptance tests (`tests/syntax/test_security_strings.ae`, `tests/syntax/test_security_stress.ae`) covering string escaping through the full pipeline, deep nesting, many variables, and long expression chains. 14 rejection and edge-case tests (`tests/integration/security/test_security_reject.sh`) covering compile-time rejection of hidden/sealed variable access, 10,000-char identifiers and string literals, and runtime correctness of embedded quotes, backslashes, percent signs, and string interpolation with special characters. Informed by vulnerability patterns found in Zig, Hare, Nim, and Odin compiler codebases.

- **`os_run` and `os_run_capture` — argv-based child process launch in `std.os`.** Spawn a child process directly via `fork` + `execvp` + `waitpid` (POSIX) without any shell in the loop. Argument items are passed verbatim, so paths-with-spaces, `$variables`, `|`, `;`, `*`, and other shell metacharacters in arguments are NOT interpreted as shell metachars. `os_run(prog, argv, env)` returns the child's exit code; `os_run_capture(prog, argv, env)` returns the child's stdout as a heap-allocated string. `argv` and `env` are Aether lists; `env` may be `null` to inherit the parent's environment. Eliminates a class of quoting bugs in `os_system` callers and is the single biggest building block for getting Aether-language tools running unmodified on Windows (Windows backend stubbed for now). Tests: `tests/syntax/test_os_run.ae` (10 sub-cases including special-character argv, captured output > 1 KB, custom env, and PATH resolution).

### Fixed

- **Closure environment lifetime bugs — five fixes landing together**: the closure/DSL feature shipped with four independent bugs in capture handling and env lifetime. All are now fixed with no regressions in `make test-ae` (181 passed / 3 failed pre-existing) and seven new regression tests under `tests/syntax/test_closure_*`.
  (1) **Captured `ptr` parameters typed `int`** (`compiler/codegen/codegen_expr.c`). `lookup_var_c_type` walked only top-level `AST_VARIABLE_DECLARATION` nodes across all functions and returned the first match by name. A closure that captured an `n: ptr` parameter of its enclosing function got `int n` in the env struct — pointer truncated on store, segfault on deref. Worse, the lookup silently returned the type of any unrelated same-named variable in another function, so correctness was name-collision-dependent. Fix threads the enclosing function name through `discover_closures` into each `ClosureInfo.parent_func` (field existed but was always NULL), then `lookup_var_c_type` searches that function's parameters (`AST_PATTERN_VARIABLE`) and locals in nested blocks first.
  (2) **Mutable captures miscompiled** (`compiler/codegen/codegen_expr.c`, `codegen_stmt.c`). Any `count = count + 1` inside a closure body emitted a shadowing local that wrote to uninitialised stack — silent wrong answers, even in-scope. Two collaborating problems: `is_local_var` treated assignment targets as fresh locals, and the closure prologue unconditionally aliased each capture to a read-only local. Fix keeps the alias pattern for read-only captures (zero cost) but detects captures assigned to in the body and routes their reads/writes through `_env->` directly. Scope analysis distinguishes captures from fresh body-locals while honouring Python-style `x = expr` shadowing when the RHS does not read `x`.
  (3) **Escaping-closure use-after-free** (`compiler/codegen/codegen.c`, `codegen_stmt.c`). A closure variable `bump = || { ... }` pushed an unconditional `free(bump.env)` onto the defer stack; at return time the compiler emitted every defer before returning, including the env-free of the very closure being returned. The caller received a closure whose env pointer was already freed — silent garbage or segfault depending on allocator reuse. Fix walks the return expression at return-emission time, collects every closure variable that appears (including `box_closure` wrappers) and transitively any closure vars they capture, then `emit_all_defers_protected` skips the matching env-free defers and emits a `/* deferred (suppressed: escapes via return) */` marker in their place.
  (4) **Closure return types hardcoded to `int`/`void`** (`compiler/codegen/codegen_expr.c`). The static `_closure_fn_N` wrapper was typed `int` (or `void`) regardless of what the closure actually returned. Any closure returning a string or pointer miscompiled: `-Wint-conversion` and a truncated return value, or the caller cast through int and dereferenced a truncated pointer. Fix picks the return type from the returned expression's `node_type`; for `return call(<captured_closure>)` chains (the typechecker leaves `call()` as `TYPE_INT` by default) it resolves through the captured closure's own body.
  (5) **`call()` expression `node_type` stuck at `TYPE_INT`** (`compiler/codegen/codegen_expr.c`). The global `call` builtin is symbol-table-typed as `TYPE_INT`, so the typechecker set `node_type=TYPE_INT` on every `call(x)` expression regardless of what `x`'s closure actually returned. `print`/`println` then picked `"%d"` for calls that really returned strings. Fix adds a post-discovery traversal that walks the program and, for every `call(<known_closure_var>)` expression, rewrites the `node_type` to match the closure's actual return type — and back-propagates into variable declarations whose initializer is such a call, so `s = call(w)` declares `s` with the right C type. Also extends `closure_var_map` seeding to inherit the closure id through `w = f()` when `f`'s body ends `return <closure_var>` — so the main→build_pair→wrapped chain resolves correctly.
  Investigation narrative and repro analysis preserved in `docs/closure-lifetime-bugs.md`. Before the fixes, four of seven representative patterns crashed or printed garbage; after, all seven print the expected output. `contrib/aether_ui/example_calculator.ae` uses named mutating closures and was a direct victim of bug (2); `examples/calculator-tui.ae` uses ref cells and was already working, and still does.

- **`lookup_qualified_symbol` NULL pointer crash**: calling `lookup_qualified_symbol(NULL, ...)` or `lookup_qualified_symbol(table, NULL)` caused a segfault — added a NULL guard at function entry. Found by the new `test_security.c` test suite.

- **Compiler codegen: imported-module functions emitted without `static` storage class**: when a `.ae` file `import`s another module, `module_merge_into_program` clones the imported function definitions into the current program's AST and codegen emits them as global C functions. If multiple `.o` files were then linked together (e.g. a multi-module build where several user files all import the same SDK module), every `.o` carried its own copy of the helpers and the linker saw them as duplicate symbols. GNU `ld` accepted `-Wl,--allow-multiple-definition` as a workaround, but Apple's `ld64` rejects that flag outright, so any aetherBuild project on macOS that imported a shared SDK module from more than one translation unit failed the link with 100+ duplicate-symbol errors. Four-part fix: (1) new `is_imported` field on `ASTNode` in `compiler/ast.h`, zero-initialised in `create_ast_node` and propagated through `clone_ast_node`; (2) `module_merge_into_program` (`compiler/aether_module.c`) sets `clone->is_imported = 1` on every function cloned out of an `import X` statement; (3) `generate_function_definition` and `generate_combined_function` in `compiler/codegen/codegen_func.c` emit `static ` before the return type when `func->is_imported` is set, so each translation unit gets a private copy of the helper that the linker never sees as a duplicate; (4) forward-declaration emission in `compiler/codegen/codegen.c` matches the storage class, otherwise C rejects the file with "static declaration follows non-static declaration". End-to-end verified on macOS arm64 with aetherBuild's `itests/rust-workspace-demo` multi-crate test: previously failed with 110 duplicate symbols at link time, now links and runs `cargo test --workspace` to completion. Linux / GNU ld builds are not affected because the old behaviour linked cleanly there (at the cost of silent symbol-table bloat).
