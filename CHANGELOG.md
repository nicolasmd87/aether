...
- **`aetherc --emit=lib` — compile an `.ae` file to a shared library (`.so`/`.dylib`) with stable C-ABI entry points**: every top-level Aether function becomes an `aether_<name>(...)` export with fixed-width argument types (`int32_t`, `int64_t`, `float`, `const char*`, plus an opaque `AetherValue*` for composite returns). The intended use is embedding Aether scripts as config/rules in host applications written in C, Java (Panama), Python, Go, etc. — the host dlopens the library and calls the script's functions like any other C API. Composite returns (maps, lists, nested structures) are walked via a small public header `runtime/aether_config.h` that declares `aether_config_get_{string,int,int64,float,bool,map,list,has}` and list equivalents — the accessors delegate to the existing `map_get` / `list_get` runtime functions and return borrowed handles tied to the root's lifetime. `aether_config_free` releases the root. Library mode is capability-empty by default: `import std.net`, `std.http`, `std.tcp`, `std.fs`, and `std.os` are rejected at compile time because an embedded library shouldn't have ambient network / filesystem / process access — the host mediates I/O. `ae build --emit=lib` wires the flag through and invokes gcc with `-fPIC -shared`; the default output is `lib<name>.so` / `.dylib`. `ae build --emit=both` is not yet implemented (a v1 limitation; run `ae build --emit=exe` and `ae build --emit=lib` separately when both artifacts are needed from the same source). Functions whose parameter or return types don't have an ABI mapping (tuples, structs, closures, actor refs) still compile but don't get an `aether_<name>` alias — `aetherc` prints a warning naming the skipped function, and the script can use those types internally. A minimal SWIG interface `runtime/aether_config.i` is shipped so `swig -python`, `-java`, `-ruby`, `-go`, `-csharp` etc. generate idiomatic bindings without further work (users author their own per-project `.i` for the `aether_<name>` entry points). Coverage: `tests/integration/emit_lib/` (primitive round-trip through dlopen), `emit_lib_composite/` (nested map + list + sub-map + nested string list walked end-to-end via the accessors), `emit_lib_lists/` (list-return including empty and out-of-range access), `emit_lib_primitives/` (`long`, `bool`, `float` boundary crossing), `emit_lib_unsupported/` (warning + stub-skip path for non-ABI types), `emit_lib_banned/` (all five capability-heavy imports rejected; control check that `std.map` still compiles), `emit_lib_dual_build/` (same source built as exe and as lib with correct symbol tables in each), and `emit_lib_swig/` (SWIG Python round-trip; skips cleanly when `swig` or python3-dev aren't installed). Full design rationale in `docs/emit-lib.md`; the configuration-as-a-library and rules-engine use cases in `docs/aether-as-a-config-language-for-other-languages.md` and `docs/aether-dsl-as-a-rules-engine.md` now have a concrete v1 foundation.
