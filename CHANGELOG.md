...
- **`aether_panic.h` sigjmp_buf shim extended to Emscripten.** The shim that maps `aether_sigjmp_buf` / `AETHER_SIGSETJMP` / `AETHER_SIGLONGJMP` to the signal-mask-unaware `jmp_buf` family was guarded by `#ifdef _WIN32` only. Emscripten supports `setjmp` / `longjmp` via `-mllvm -enable-emscripten-sjlj` but not `sigsetjmp` / `siglongjmp`, and its POSIX signal surface has no `sigaction` / `SA_SIGINFO`. The guard is now `#if defined(_WIN32) || defined(__EMSCRIPTEN__)`. The matching `aether_panic_install_signal_handlers()` implementation is now gated `#if !defined(_WIN32) && !defined(__EMSCRIPTEN__)` with a no-op stub for the other path. The rest of the panic/try/catch path (built on `setjmp` / `longjmp`) works unchanged on wasm.

### Changed

- **`docs/containment-sandbox.md` — host-module matrix extended with a Tcl column and a new "Shared-interpreter caveats" section.** The caveats section is the proper home for the long-standing design quirks Perl, Ruby, and Tcl exhibit (shared interpreter retains scrubbed env across run/run_sandboxed boundaries) plus Ruby's cosmetic `Fiddle.dlopen` surprise. Keeps this information out of user-facing per-module READMEs (which should stay minimal) and puts it in the one design-level reference doc where sandbox users already look for behavior detail.
