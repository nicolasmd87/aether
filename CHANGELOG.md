...
- **`docs/json-parser-design.md`** — new design-rationale doc covering arena memory model, character classification LUT, UTF-8 DFA, SIMD string fast-loop (with its SSE2/NEON/scalar dispatch), number-parser three-path structure, container representation, error reporting, depth limit, thread safety, and the "remaining gap vs. yyjson" discussion (tape representation as deferred work with a concrete dual-representation design).
