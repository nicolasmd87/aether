...
- nginx-class **reverse proxy** in stdlib — `std.http.proxy` forwards inbound requests to a pool of upstream servers with weighted load balancing (round-robin / least-conn / ip-hash / smooth weighted RR), active health checks, in-memory LRU response cache with RFC 7234 cacheability rules, and per-upstream circuit breaker. Hop-by-Hop header handling per RFC 7230. See [Reverse Proxy](docs/http-reverse-proxy.md).
