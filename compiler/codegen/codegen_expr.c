                    ...
                    for (int i = 0; i < expr->child_count; i++) {
                        ...
                        if (expected == TYPE_PTR && arg->node_type &&
                            ...
                            fprintf(gen->output, ")");
                                   ...
                                   !is_aether_extern_func(gen, func_name)) {
                            ...
                            //      str_data/str_len internally — those need
                            //      the header pointer to recover the stored
                            //      length on binary content. See
                            //      is_stdlib_string_aware_extern below.
                            //   2. `@aether extern foo(...)` — extern is
                            //      Aether-emitted C, the receiver dispatches
                            //      on AetherString magic via str_len. Without
                            //      this, binary content with embedded NULs
                            //      strlen-truncates at the boundary (#351).
                            // Closes #297.
