...
// Try to resolve an import path. If the full path fails to resolve as a module,
// split at the last dot and treat the tail as a selective-import symbol (Java-style
// `import java.util.List` — class name tacked on the end). On success with split,
// mutates the import_node in place: shortens `value` to the module path and inserts
// the symbol name as an AST_IDENTIFIER first child (matching the existing selective
// import shape from `import mod (a, b)`). Caller owns the returned file path.
...
void module_merge_into_program(ASTNode* program) {
    ...
    // Second pass: for each selective import, rewrite bare-name references in
    // user code (not the imported clones) to their module-prefixed form.
    // This lets callers write `button("1")` instead of `aether_ui.button("1")`
    // after `import contrib.aether_ui.button` or `import x (a, b, c)`.
    for (int i = 0; i < program->child_count; i++) {
