...
if ! AETHER_HOME="" "$ROOT/build/ae" build --namespace . >"$TMPDIR/build.log" 2>&1; then
...
elif [ ! -f "$TMPDIR/com/example/calc/Calc.java" ]; then
    echo "  [FAIL] Calc.java was not generated"; find "$TMPDIR" -type f; fail=$((fail + 1))
else
    LIB_PATH=""
    ...
    elif ! javac com/example/calc/Calc.java Check.java >"$TMPDIR/javac.log" 2>&1; then
